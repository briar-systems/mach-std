...
subprocess.run(['git', '-C', str(compiler), 'checkout', '--detach', '3a962411cdf278e8be9fcf7210308b806baec6c1'], check=True)
...
(evidence / 'audited-source.json').write_text(json.dumps(dict(mach='3a962411cdf278e8be9fcf7210308b806baec6c1', std=actual_std, fixpoint=True)))
...
(evidence / 'debug-image-status.json').write_text(json.dumps(dict(returncode=result.returncode, compiler='3a962411cdf278e8be9fcf7210308b806baec6c1', profile='debug', opt=0, debug=True)))
with Path(os.environ['GITHUB_ENV']).open('a') as output:
    output.write('MACH_583_COMPILER=' + str(compiler / 'b') + '\n')
    output.write('MACH_583_PROFILE=release\n')
