...
`thread_block() *ThreadBlock` returns the calling thread's block without a system call, or nil where none is installed. `thread_block_enter(block: *ThreadBlock) i64` installs a block for the calling thread and writes its `self_addr`. The runtime installs a static block on the main thread before `main`, and `thread_spawn` installs one it owns on every thread it creates before the thread runs std code, so `std.sync.thread.current_token` is valid on both. linux holds the block in the thread pointer (`CLONE_SETTLS` at spawn, `arch_prctl(ARCH_SET_FS)` on x86_64, `tpidr_el0` on aarch64, `tp` on riscv64) and keeps a spawned thread's block at the top of its stack. std links no C runtime on linux, so nothing else reads that register. darwin's thread register belongs to libpthread, so the block rides a pthread key, and windows's rides a TLS index read straight from the TEB. The block is not general thread-local storage. It is where per-thread state std owns, such as stack bounds, belongs. A user-supplied implementation that has no thread pointer may return nil from `thread_block`, and the token then falls back to the thread id.
