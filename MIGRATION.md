...
| suite at this phase | 1213 passed, 0 failed under the v5 compiler `8464568d` on linux-x86_64 after S3c (1199 after S3a (1185 at S1 phase 2, 1182 at S1 phase 1 under `9a15ac3a6`, 1157 on the base under both the 4.30.0 seed and the v5 compiler) |
...
| `process.exec.Error` | `pub tag Error: u8 { native: Failure; output: OutputFailure; alloc: A.Error; env: env.EnvError; empty_name; unset; not_found; ungrouped; unsupported; }` | yes |
...
- `origin/feat/618` (four commits): `dac63cd` directory cursors across native
  ...
  `O.Option[ExitStatus]` becomes `opt[ExitStatus]`). `9dd151d` landed that
  way with S3c (`process.exec.Error`, `os.ProcessStatus`, the
  `test/process-status` CI leg); `dac63cd` and `a230488` are S3b's.
